...
<div x-data="kioskSystem" x-cloak class="flex-grow flex flex-col">
        ...
        <div class="flex-grow flex flex-col items-center justify-center p-4" 
            ...
            <div class="text-center mb-8 sm:mb-16 relative">
                ...
                <div class="relative inline-flex flex-col items-center gap-4 sm:gap-6">
                    <div class="w-16 h-16 sm:w-24 sm:h-24 bg-white border-2 sm:border-4 border-slate-900 shadow-[4px_4px_0px_#2563eb] sm:shadow-[8px_8px_0px_#2563eb] rounded-xl sm:rounded-2xl flex items-center justify-center p-3 sm:p-4 animate-float-gentle relative">
                        <img src="{{ asset('images/logo.png') }}" alt="Logo" class="max-w-full max-h-full object-contain">
                        <!-- Tiny burst badge -->
                        <div class="absolute -top-3 -right-3 sm:-top-4 sm:-right-4 w-8 h-8 sm:w-10 sm:h-10 bg-blue-600 comic-burst border-2 border-slate-900 flex items-center justify-center rotate-12">
                            <span class="text-[6px] sm:text-[8px] font-black text-white italic">KIOSK</span>
                        </div>
                    </div>
                    <div class="space-y-1 sm:space-y-2">
                        <h1 class="text-3xl sm:text-5xl md:text-7xl font-black tracking-tighter text-slate-900 leading-none uppercase italic chromatic-offset">PERPUSTAKAAN</h1>
                        <p class="text-[10px] sm:text-xl font-black text-blue-600 tracking-[0.2em] sm:tracking-[0.4em] uppercase italic flex items-center justify-center gap-2 sm:gap-4">
    ...
    <template x-if="isKioskActive">
        <div class="flex-grow flex flex-col space-y-10 animate-fade-in px-2 sm:px-0">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-6 bg-white panel-border p-8 rounded-3xl relative overflow-hidden">
                <div class="absolute right-0 top-0 h-full w-[30%] bg-blue-100/50 rotate-12 -mr-10 -mt-10 blur-3xl benday-dots"></div>
                
                <div class="relative z-10 flex items-center gap-6">
                    <div class="h-16 w-16 bg-white border-2 border-slate-900 rounded-xl flex items-center justify-center p-3 shadow-[4px_4px_0px_#1e293b]">
                        <img src="{{ asset('images/logo.png') }}" alt="Logo" class="max-w-full max-h-full object-contain">
                    </div>
                    <div>
                        <div class="flex items-center gap-3">
                            <h2 class="text-2xl font-black text-slate-900 tracking-tighter uppercase italic leading-none chromatic-offset">OPERATOR_TERMINAL</h2>
                            <span class="bg-blue-600 text-white text-[10px] font-black px-3 py-1 rounded border-2 border-slate-900 uppercase tracking-widest shadow-[3px_3px_0px_#1e293b]">ACTIVE</span>
                        </div>
                ...
                <div class="flex items-center gap-6 relative z-10">
                    <div x-show="user" class="hidden sm:flex items-center gap-3 px-4 py-2 bg-slate-900 text-white rounded-lg border-2 border-slate-900 font-black italic text-xs shadow-[4px_4px_0px_#2563eb]">
                        <span class="text-slate-400 uppercase text-[9px]">TIMER:</span>
                        <span class="text-blue-400" x-text="Math.ceil(timer / 10) + 's'"></span>
                    </div>
                    <button @click="closeKiosk()" class="px-6 py-2 bg-white border-[3px] border-red-600 text-red-600 hover:bg-red-600 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-[4px_4px_0px_#ef4444] hover:shadow-none active:translate-x-1 active:translate-y-1">TERMINATE</button>
                </div>
            </div>
            ...
            <div class="flex-grow flex flex-col">
                ...
                <template x-if="step === 'login'">
                    <div class="flex-grow flex flex-col lg:flex-row gap-12 items-stretch" x-transition>
                        <!-- Method Select -->
                        <div class="flex-grow flex flex-col justify-center space-y-12 lg:w-1/2">
                            <div class="space-y-4">
                                <h2 class="text-4xl sm:text-7xl font-black text-slate-900 tracking-tighter leading-tight uppercase italic chromatic-offset">USER_AUTH_GATEWAY</h2>
                                <p class="text-slate-400 font-black uppercase tracking-[0.3em] italic">Access secure library services using your digital credentials.</p>
                            </div>

                            <div class="space-y-6 max-w-lg">
                                <button @click="loginMethod = 'qr'" 
                                        :class="loginMethod === 'qr' ? 'bg-white border-blue-600 shadow-[10px_10px_0px_#2563eb]' : 'bg-slate-50 border-slate-200 opacity-60'" 
                                        class="w-full p-8 rounded-3xl flex items-center justify-between border-4 transition-all duration-500 group relative overflow-hidden">
                                    <div class="absolute inset-0 opacity-[0.03] benday-dots"></div>
                                    <div class="flex items-center gap-6 relative z-10">
                                        <div :class="loginMethod === 'qr' ? 'bg-blue-600 text-white border-slate-900' : 'bg-slate-100 text-slate-400 border-slate-200'" class="p-5 rounded-2xl border-2 shadow-[4px_4px_0px_#1e293b] transition-colors">
                                            <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" /></svg>
                                        </div>
                                        <div class="text-left">
                                            <p class="font-black text-slate-900 text-2xl uppercase italic tracking-tighter">Badge_Optic_Scan</p>
                                            <p class="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] mt-1">Instant Verification</p>
                                        </div>
                                    </div>
                                    <div x-show="loginMethod === 'qr'" class="w-4 h-4 rounded-full bg-blue-600 animate-ping relative z-10"></div>
                                </button>

                                <button @click="loginMethod = 'manual'" 
                                        :class="loginMethod === 'manual' ? 'bg-white border-indigo-600 shadow-[10px_10px_0px_#4f46e5]' : 'bg-slate-50 border-slate-200 opacity-60'" 
                                        class="w-full p-8 rounded-3xl flex items-center justify-between border-4 transition-all duration-500 group relative overflow-hidden">
                                    <div class="absolute inset-0 opacity-[0.03] benday-dots"></div>
                                    <div class="flex items-center gap-6 relative z-10">
                                        <div :class="loginMethod === 'manual' ? 'bg-indigo-600 text-white border-slate-900' : 'bg-slate-100 text-slate-400 border-slate-200'" class="p-5 rounded-2xl border-2 shadow-[4px_4px_0px_#1e293b] transition-colors">
                                            <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </div>
                                        <div class="text-left">
                                            <p class="font-black text-slate-900 text-2xl uppercase italic tracking-tighter">Manual_Secure_Code</p>
                                            <p class="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] mt-1">Keypad Entry</p>
                                        </div>
                                    </div>
                                    <div x-show="loginMethod === 'manual'" class="w-4 h-4 rounded-full bg-indigo-600 animate-ping relative z-10"></div>
                                </button>
                ...
                <template x-if="step === 'action_selection'">
                    <div class="flex-grow flex flex-col items-center justify-center py-12 animate-fade-in" x-transition>
                        <div class="text-center mb-16 relative">
                            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[140%] h-16 zig-zag opacity-5 -rotate-3"></div>
                            <h2 class="text-4xl sm:text-7xl font-black text-slate-900 tracking-tighter uppercase italic mb-6 relative z-10 chromatic-offset">CORE_MODULE_SELECT</h2>
                            <p class="text-slate-400 font-black uppercase tracking-[0.3em] italic relative z-10">Validated_User: <span class="text-blue-600 border-b-4 border-slate-900 pb-1" x-text="user?.name"></span></p>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 w-full max-w-6xl">
                            <!-- Borrow -->
                            <button @click="startAction('borrow')" class="group bg-white/90 backdrop-blur-xl panel-border p-12 rounded-[3.5rem] text-left hover:border-blue-600 hover:shadow-[16px_16px_0px_#2563eb] transition-all duration-500 relative overflow-hidden">
                                <div class="absolute top-0 right-0 w-48 h-48 bg-blue-100/50 -mr-16 -mt-16 rounded-full blur-3xl benday-dots transition-all"></div>
                                
                                <div class="w-24 h-24 bg-blue-600 text-white rounded-2xl flex items-center justify-center mb-10 border-4 border-slate-900 shadow-[6px_6px_0px_#1e293b] group-hover:scale-110 transition-transform relative z-10">
                                    <svg class="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5S19.832 5.477 21 6.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                                </div>
                                <h3 class="text-4xl font-black text-slate-900 uppercase italic tracking-tighter mb-4 relative z-10 chromatic-offset">BORROW_ITEM</h3>
                                <p class="text-slate-400 text-sm font-black uppercase tracking-widest leading-relaxed pr-8 relative z-10 italic">Initiate new cryptographic loan transaction. Scans required for validation.</p>
                                <div class="mt-10 flex items-center gap-4 text-blue-600 text-[10px] font-black uppercase tracking-[0.4em] relative z-10">
                                    <span class="bg-slate-900 text-white px-4 py-2 border-2 border-slate-900 shadow-[3px_3px_0px_#1e293b]">RUN_WORKFLOW</span><svg class="w-6 h-6 group-hover:translate-x-3 transition-transform text-slate-900" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                                </div>
                            </button>

                            <!-- Return -->
                            <button @click="startAction('return')" class="group bg-white/90 backdrop-blur-xl panel-border p-12 rounded-[3.5rem] text-left hover:border-indigo-600 hover:shadow-[16px_16px_0px_#4f46e5] transition-all duration-500 relative overflow-hidden">
                                <div class="absolute top-0 right-0 w-48 h-48 bg-indigo-100/50 -mr-16 -mt-16 rounded-full blur-3xl benday-dots transition-all"></div>

                                <div class="w-24 h-24 bg-indigo-600 text-white rounded-2xl flex items-center justify-center mb-10 border-4 border-slate-900 shadow-[6px_6px_0px_#1e293b] group-hover:scale-110 transition-transform relative z-10">
                                    <svg class="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M11 15l-3-3m0 0l3-3m-3 3h8M3 12a9 9 0 1118 0 9 9 0 01-18 0z" /></svg>
                                </div>
                                <h3 class="text-4xl font-black text-slate-900 uppercase italic tracking-tighter mb-4 relative z-10 chromatic-offset">RETURN_ITEM</h3>
                                <p class="text-slate-400 text-sm font-black uppercase tracking-widest leading-relaxed pr-8 relative z-10 italic">Deactivate active loan session. Items must be optically verified.</p>
                                <div class="mt-10 flex items-center gap-4 text-indigo-600 text-[10px] font-black uppercase tracking-[0.4em] relative z-10">
                                    <span class="bg-slate-900 text-white px-4 py-2 border-2 border-slate-900 shadow-[3px_3px_0px_#1e293b]">VERIFY_STATE</span><svg class="w-6 h-6 group-hover:translate-x-3 transition-transform text-slate-900" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                                </div>
                            </button>
                        </div>
                        
                        <button @click="logout()" class="mt-16 text-[10px] font-black text-slate-400 hover:text-red-600 uppercase tracking-[0.4em] transition-colors border-b-2 border-transparent hover:border-red-600 pb-1 italic">TERMINATE_CURRENT_IDENTITY</button>
                    </div>
                </template>
                ...
                <template x-if="step === 'book_input'">
                    <div class="flex-grow flex flex-col animate-fade-in" x-transition>
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 w-full max-w-6xl mx-auto flex-grow items-start">
                            <!-- Left: Details & Manual Input -->
                            <div class="bg-white/90 backdrop-blur-xl panel-border p-12 rounded-3xl relative overflow-hidden group">
                                <div class="absolute top-0 right-0 w-48 h-48 bg-blue-100/40 -mr-16 -mt-16 rounded-full blur-3xl benday-dots transition-all"></div>
                                
                                <div class="relative z-10 space-y-8">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-5">
                                            <div class="bg-blue-600 text-white p-4 rounded-xl border-2 border-slate-900 shadow-[4px_4px_0px_#1e293b]">
                                                <svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 12h6m-6 4h6m2 5H7a2 2 0 00-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                            </div>
                                            <div>
                                                <h2 class="text-2xl font-black text-slate-900 uppercase tracking-tighter italic chromatic-offset">ITEM_IDENTIFICATION</h2>
                                                <div class="h-2 w-16 bg-blue-600 mt-1"></div>
                                            </div>
                                        </div>
                                        <button @click="step = 'action_selection'" class="bg-white border-2 border-slate-900 p-3 rounded-xl shadow-[3px_3px_0px_#ef4444] hover:shadow-none active:translate-x-1 active:translate-y-1 transition-all group">
                                            <svg class="w-5 h-5 text-red-600 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12" /></svg>
                                        </button>
                                    </div>

                                    <div class="space-y-6">
                                        <div class="space-y-3">
                                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 block">IDENTIFICATION_CODE / SERIAL</label>
                                            <div class="flex bg-slate-50 border-4 border-slate-900 rounded-2xl overflow-hidden shadow-[8px_8px_0px_#2563eb]">
                                                <div class="bg-slate-100 text-slate-900 px-6 py-5 flex items-center border-r-4 border-slate-900">
                                                    <span class="text-[10px] font-black uppercase tracking-tighter italic">DATA_ID:</span>
                                                </div>
                                        ...
                                        <button @click="processBookManual()" 
                                                ...
                                                class="w-full py-6 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-black text-xl shadow-[8px_8px_0px_#2563eb] transition-all active:translate-x-1 active:translate-y-1 active:shadow-none disabled:opacity-50 uppercase italic tracking-widest">
                                            VERIFY_ITEM_DATA
                                        </button>
                                    </div>
                ...
                <template x-if="step === 'finished'">
                    <div class="flex-grow flex flex-col items-center justify-center p-12 text-center space-y-16 animate-fade-in" x-transition>
                        <div class="relative">
                            ...
                            <div class="absolute -inset-24 bg-blue-600/10 comic-burst animate-pulse blur-xl"></div>
                            <div class="w-60 h-60 bg-white border-4 border-slate-900 rounded-[3rem] flex items-center justify-center text-slate-900 relative z-10 shadow-[24px_24px_0px_#2563eb] rotate-3">
                                <svg class="w-32 h-32" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                <!-- Floating onomatopoeia -->
                                <div class="absolute -top-8 -right-12 animate-bounce">
                                    <span class="ONOMATOPOEIA text-4xl rotate-12">YES!</span>
                                </div>
                            </div>
                        </div>
                        
                        <div class="space-y-6 relative z-10">
                            <h2 class="text-6xl sm:text-8xl font-black text-slate-900 tracking-tighter uppercase italic chromatic-offset">TASK_SUCCESS.</h2>
                            <p class="text-slate-400 text-xl font-black tracking-widest uppercase italic">The cryptographic transaction has been logged and finalized.</p>
                        </div>

                        <div class="flex flex-wrap justify-center gap-10 relative z-10">
                            <button @click="logout()" class="px-20 py-8 bg-slate-900 text-white rounded-3xl font-black text-2xl hover:bg-slate-800 transition-all shadow-[12px_12px_0px_#2563eb] active:translate-x-1 active:translate-y-1 active:shadow-none uppercase italic tracking-wider">CLOSE_SESSION</button>
                            <button @click="step = 'action_selection'" class="px-20 py-8 bg-white border-4 border-slate-900 text-slate-900 rounded-3xl font-black text-2xl hover:bg-slate-50 transition-all uppercase italic tracking-wider shadow-[12px_12px_0px_#1e293b] active:translate-x-1 active:translate-y-1 active:shadow-none">NEW_BATCH</button>
                        </div>
                    </div>
                </template>
